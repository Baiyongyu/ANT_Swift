...
class ItemCell: UICollectionViewCell {
    ...
    var isHiddenBar: Bool = true {
        didSet {
            toggleDisplayBottomBar(isHidden: isHiddenBar)
        }
    }
...
//    @IBOutlet weak var layoutView: UIView!
    ...
    @IBOutlet weak var asHUD: UIView!
...
extension ItemCell: UIScrollViewDelegate {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        doubleTapGesture = UITapGestureRecognizer(target: self, action: #selector(ItemCell.doubleTap(tapG:)))
        doubleTapGesture.numberOfTapsRequired = 2
        doubleTapGesture.numberOfTouchesRequired = 1
        
        singleTapGesture = UITapGestureRecognizer(target: self, action: #selector(ItemCell.singleTap(tapG:)))
...
//        asHUD.type = NVActivityIndicatorType.BallTrianglePath
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(60)) {
            DispatchQueue.main.async {
                if self.imageV.image != nil {
                    self.imageIsLoaded(img: self.imageV.image!, needAnim: false)
                }
            }
        }
    }
    ...
    func singleTap(tapG: UITapGestureRecognizer){
        ...
        if scrollView.zoomScale > 1 {
            doubleTap(tapG: doubleTapGesture)
        }else {
    ...
    func dataFill() {
        ...
        if photoType == PhotoBrowser.PhotoType.Local {
            ...
            cache.fetch(key: photoModel.hostHDImgURL,  failure: {[unowned self] fail in
                ...
                self.cache.fetch(URL: URL.init(string: self.photoModel.hostHDImgURL)!, formatName: "", failure: { (fail) in
                    print("失败\(String(describing: fail))")
                }, success: { [unowned self] img in
                    if !UserDefaults.standard.bool(forKey: CFPBShowKey) {
                        return
                    }
                    ...
                    if self.photoModel?.excetionFlag == false {
                        return
                    }
                    
                    if self.photoModel.modelCell !== self {
                        return
                    }
                    
                    self.hasHDImage = true
                    self.dismissAsHUD(needAnim: true)
                ...
                self.dismissAsHUD(needAnim: false)
